...
let numero_juego = prompt("Ingrese mediante un numero entero, el numero de veces que desea jugar: "); // crear la variable e imput
let contador = 0 //contador para comparar variables 

while(contador < numero_juego){ // ciclo para dar cantidad de juegos 
    let jugada_humno = prompt("Ingrese su jugada. Debe ser piedra,papel o tijera","piedra")
    let ganador = definirGanador (jugada_humno, jugadaMaquina()) //se crea una funcion con las variables de jugada humano y jugada maquina
    contador++

    switch (ganador){ // switch en vez de if anidado con la opcion valida
        case 'humano':
            document.write(`<div class="alert alert-success" role="alert">Felcidades humano ganaste</div> `)
            break;
        case 'maquina':
            document.write(`<div class="alert alert-danger" role="alert">Humano, Perdiste</div>`)
            break;
        default:
            document.write(`<div class="alert alert-warning" role="alert">Empate</div>`)
            break;
    }

    function jugadaMaquina(){ //funcion con retornos//
        switch(Math.floor(Math.random()*3)){
            case 0:
                return 'piedra'; //si el valor es 0 me retorna un piedra//
                break;
            case 1:
                return 'papel'; // si el valor es 1 me retorna un papel//
                break;
            case 2:
                return 'tijera'; // si el valor es 2 me retorna un tijera
                break;
        }
    }

    function definirGanador(humano,maquina){ // la funcion definir ganador tiene las variables de (jugada_humano y jugada maquina) y luego son comparadas con las variables locales de humano y maquina.
        let resultado = ''; //se determina una variable local, solo funciona en esta funcion//
        if(humano == maquina){
            resultado = 'empate'
        }else if((humano == 'tijera' && maquina == 'papel') || (humano == 'piedra' && maquina=='tijera') || (humano == 'papel' && maquina == 'piedra')) {     
            resultado = 'humano'
        }else{
            resultado = 'maquina'
        }
        return resultado //devuelve el valor que se cargó en resultado//
    }
}